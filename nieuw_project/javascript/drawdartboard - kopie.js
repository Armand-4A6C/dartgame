...
function drawNumbers(ctx, radius) {
    ...
    ctx.font = (canvas.height / 2) * 0.10 + "px arial";
    ...
    if (i < 6) {
        ...
        ctx.rotate(ang);
        ctx.translate(0, - radius * 0.97 + canvas.offsetLeft);
        ctx.rotate(- ang);
...
// ctx.clearRect(0, canvas.width, 0, canvas.height, 0)
//
...
function drawAll() {
    ...
    ctx.clearRect(0, canvas.width, 0, canvas.height, 0)
    drawDartboard(scoreE)
    drawNumbers(ctx, (canvas.width * 0.68));
    dartStyle();
}
drawAll();
